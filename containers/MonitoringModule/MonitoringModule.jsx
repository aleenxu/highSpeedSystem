...
import DetailsPop from '../../components/DetailsPop/DetailsPop'
...
class MonitoringModule extends React.Component {
  constructor(props) {
    ...
    this.endTimeUrl = '/control/control/plan/update/end/time/' // {eventTypeId}/{eventId}/{controllId} 修改管控方案结束时间'
    ...
    this.promptUrl = '/control/control/plan/disable/timeout/notify/' // {eventId} 添加不再提示案件'
    ...
    this.socketUrl = process.env.NODE_ENV === 'development' ? 'ws://192.168.1.124:20207/control/notify/traffic/' : 'ws://47.108.61.84:8081/control/notify/traffic/'
  }
  componentWillMount = () => {
    // 获取用户权限
    this.handlelimitArr()
  ...
  handleEventPopup = (type, boolean) => {
    ...
    if (type === 'Details') {
      ...
      if (boolean) {
        window.lineFlag = boolean.eventType !== 9 || boolean.markEventType !== 9
  ...
  handleSelect = (value, name, type, data) => {
    if (type === 'publishPlanVO' && (name === 'deviceControlType' || name === 'content')) {
      ...
      this.state.roadNumber.forEach((item) => {
            ...
            if (_this.state.boxFlag && _this.state.flagClose) {
              if (_this.state.EventTagPopupTit === '主动管控') {
                const params = {
                  ...
                  roadDirectionName: _this.controlDatas.directionName,
                }
  ...
  getDevice = (item) => {
    ...
    if (item && item.devices.length > 0) {
      for (let i = 0, devicesArr = item.devices; i < devicesArr.length; i++) {
        if (devicesArr[i].device) {
          devicesArr[i].device.map((items) => {
            existsDevices.push(items.appendId)
          })
        }
      }
    }
    ...
    getResponseDatas('post', this.getDeviceAllUrl, params).then((res) => {
      ...
      if (result.code === 200) {
        ...
        result.data.map((items) => {
          if (!(existsDevices.includes(items.appendId))) {
            noDevices.push(items.appendId)
          }
        })
        if (result.data.length > 0) {
          this.setState({ boxSelect: true, boxSelectList: result.data, oldDevicesList: noDevices })
        } else {
          message.info(item.roadDirectionName + '没有选中相应的硬件设备！')
  ...
  handleSocket = () => {
    ...
    ws.onmessage = (evt) => {
      ...
      if (evt.data !== '连接成功') {
        ...
        console.log(data, data.soonTimeout);
        if (data.plan) {
          let x, click
        ...
        } else if (data.soonTimeout) { // 超时提醒
          const { TimeData = [] } = this.state
          console.log(TimeData);
          let ds = true
          this.endValueTime = moment(Number(data.soonTimeout.planEndTime)).format('YYYY-MM-DD:HH:MM:SS')
          if (TimeData) {
            TimeData.forEach((item) => {
              if (item.planNum === data.soonTimeout.planNum) {
                item = { ...data.soonTimeout }
                ds = fasle
              }
              if (ds) {
            ...
            this.setState({ TimeData: [data.soonTimeout] })
          }
        }
      }
    }
  ...
  handleMarkControl = () => {
    const { controlDes, list } = this.reservePopup
    ...
    getResponseDatas('get', this.getInfoUrl + planNum).then((res) => {
      ...
      if (result.code === 200) {
        ...
        this.publishPlanVO = {
          ...
          endTime: result.data.endTime ? this.getDate(result.data.endTime) : moment().add(2, 'hour').format('YYYY-MM-DD HH:mm:ss'),
          list,
  ...
  handleEventTag = (boolean, e) => {
    ...
    this.setState({
      ...
      console.log(this.controlDatas, '11111111');
      ...
      if (this.state.detailsPopup && (!this.state.EventTagPopup)) {
        const latlngArr = JSON.parse(JSON.stringify(this.state.detailsPopup.latlng))
        setTimeout(() => {
          window.drawLine(latlngArr, window.lineFlag)
        }, 1000)
      }
  ...
  handleRelease = () => {
    ...
    console.log(this.publishPlanVO, detailsPopup);
  ...
  handleWhethe = () => {
    ...
    const { planNum } = reservePopup
    getResponseDatas('put', this.endTimeUrl + planNum + '?endTime=' + endValueTime).then((res) => {
      const result = res.data
      if (result.code === 200) {
        message.success(result.message)
  ...
  handleEndValueTimeState = (item, index) => {
    if (item) {
      ...
      this.setState({
        endValueTime: this.endValueTime || this.getDate(),
      })
    } else {
      this.setState({
        endValueTime: null,
  ...
  handleNoneTimeState = (item, index) => {
    const { planNum } = item
    getResponseDatas('put', this.promptUrl + planNum).then((res) => {
      const result = res.data
      if (result.code === 200) {
        message.success(result.message)
        const { TimeData } = this.state
          ...
          item.device.forEach((items) => {
            if (value.innerDevices) {
              value.innerDevices.forEach((it) => {
                if (items.deviceId === it.deviceId) {
                  it.content = items.content || items.showContent
                  it.deviceControlType = items.deviceControlType
                }
              })
            } else {
  ...
  handleTypeWinPopup = () => {
    const { TypeWinPopup, detailsPopup, deviceTypes, EventTagPopupTit } = this.state
    let data = []
    if (EventTagPopupTit === '主动管控') {
      data = deviceTypes
    } else {
    ...
    data.forEach((item) => {
      if (item.dictCode === TypeWinPopup.deviceType) {
        const itemData = JSON.parse(JSON.stringify(item))
        TypeWinPopup.innerDevices.forEach((ite) => {
          let bool = true
          if (itemData.device.length) {
            itemData.device.forEach((items, index) => {
              if (items.appendId === ite.appendId) {
                item.device[index] = ite
                bool = false
                console.log(ite);
              }
            })
            if (bool) {
              item.device.push(ite)
              bool = true
            }
          } else {
            item.device.push(ite)
    ...
    if (EventTagPopupTit === '主动管控') {
      this.setState({ TypeWinPopup: null, deviceTypes: data })
    } else {
      detailsPopup.devices = data
      this.setState({ TypeWinPopup: null, detailsPopup })
    }
  }
  handleInfoWinPopup = () => {
  ...
  handledisabledEndDate = (endValue) => {
    return endValue && endValue < moment(this.endValueTime)
  }
  range = (start, end) => {
    const result = []
  ...
  handleFormat = (value, key) => {
    if (key) {
      return moment(Number(value))[key]()
    } else {
      return moment(value).format('YYYY-MM-DD HH:mm:ss')
    }
  }
  disabledDateTime = (value) => {
    if (this.handleFormat(this.endValueTime) === this.handleFormat(value)) {
      const Hours = this.handleFormat(this.endValueTime, 'hour')
      const Minutes = this.handleFormat(this.endValueTime, 'minute')
      const Seconds = this.handleFormat(this.endValueTime, 'second')
      return {
                        ...
                        <div className={styles.ItemBox}>
                          <div className={styles.RowBox}>
                            ...
                            <div className={styles.right}>
                              <Button onClick={() => { this.handleNoneTimeState(item, index) }} className={styles.Button}>不再提示</Button>
                              <Button className={styles.Button} onClick={() => { this.handleEndValueTimeState(item, index) }}>延时</Button>
                            </div>
                          </div>
                        </div>
                      </div>
                              ...
                              <div className={styles.RowBox}>
                                {
                                  items.device && items.device.map((item, index) => {
                                    return (
                                      <div className={styles.InputBox} key={item.appendId}>
                                        <div className={styles.ItemInput} style={{ width: '30%', height: '32px', lineHeight: '32px', overflow: 'hidden' }} title={index + 1 + '.' + item.deviceName + '-' + item.roadDirectionName}>
                                          {
                              ...
                              <div className={styles.RowBox}>
                                {
                                  items.device && items.device.map((item, index) => {
                                    return (
                                      <div className={styles.InputBox} key={item.appendId}>
                                        <div className={styles.ItemInput} style={{ width: '30%', height: '32px', lineHeight: '32px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={index + 1 + '.' + item.deviceName + '-' + item.roadDirectionName}>
                                          {
                                            reservePopup.planStatus === 1 ?
                                              <Icon type="close-circle" className={styles.CloneItem} onClick={() => { this.handleCloseCircle(indexs, index, item) }} />
                                              : null
        ...
        }
        {
          detailsPopup ?
            <DetailsPop
              ...
              handleEventTag={this.handleEventTag}
            />
            : null
        }
        {
                                ...
                                <div>
                                  {
                                    item.device && item.device.map((items, index) => {
                                      return <div className={styles.PanelBox} key={items.appendId}><p className={styles.PanelItem} onClick={() => { this.openInfoWin(items) }}>{`${index + 1}. ${items.deviceName} ${items.roadDirectionName} `}</p>{<Icon onClick={() => { this.handleSubDetailsPopupList(ind, index) }} className={styles.MinusItem} type="close" />}</div>
                                    })
                                  }
                            ...
                            return (
                              <Panel className={styles.PanelChs} header={item.codeName} key={item.dictCode}>
                                <div>
                                  {
                                    item.device && item.device.map((items, index) => {
                                      return <div className={styles.PanelBox} key={items.appendId}><p className={styles.PanelItem} onClick={() => { this.openInfoWin(items) }}>{`${index + 1}. ${items.deviceName} ${items.roadDirectionName}`}</p>{detailsPopup.planSource > 0 || <Icon onClick={() => { this.handleSubDetailsPopupList(ind, index) }} className={styles.MinusItem} type="close" />}</div>
                                    })
                                  }
                                  {(item.device && item.device.length === 0) ? <NoData /> : null}
                                </div>
        ...
        {/* 多设备 类型展示框 */}
        {
          TypeWinPopup ?
            <TypeWinPop
              ...
              EventTagPopup={EventTagPopup}
              deviceString={this.state.deviceString}
              TypeWinPopup={TypeWinPopup}
              MeasuresList={MeasuresList}
              handleTypeWinPopup={this.handleTypeWinPopup}
              handleClose={() => { this.setState({ TypeWinPopup: null }) }}
            /> : null
        }
        {InfoWinPopup ?
          <div className={styles.MaskBox}>
            <div className={classNames(styles.EventPopup, styles.VIboardPopup)}>
              <div className={styles.Title}>{InfoWinPopup.deviceName}-{InfoWinPopup.directionName}-{InfoWinPopup.deviceTypeName}<Icon className={styles.Close} type="close" onClick={() => { this.setState({ InfoWinPopup: null }) }} /></div>
              ...
              <div className={styles.ItemFooter} style={{ bottom: '-15px' }}>
                ...
                <span onClick={() => { this.setState({ InfoWinPopup: null }) }} >返&nbsp;&nbsp;回</span>
              </div>
            </div>
          </div> : null}
